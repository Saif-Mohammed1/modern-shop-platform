...
const rule = {
    meta: {
        ...
        messages: {
            noNestedButtons: "Nested buttons are not allowed. Found {{ parent }} > {{ child }}.",
        },
        schema: [],
    },
    defaultOptions: [],
    create(context) {
        function isButtonElement(node) {
            ...
            if (name.type === "JSXMemberExpression") {
                const { property } = name;
                return (property.type === "JSXIdentifier" &&
                    property.name.toLowerCase().endsWith("button"));
            }
            return false;
        }
        ...
        return {
            JSXOpeningElement(node) {
                ...
                while (parent && parent.type !== types_1.AST_NODE_TYPES.Program) {
                    if (parent.type === types_1.AST_NODE_TYPES.JSXElement &&
