...
export class TwoFactorRepository extends BaseRepository<ITwoFactorAuth> {
  ...
  async deleteTwoFactor(userId: string, session?: ClientSession): Promise<boolean> {
    const result = await this.model.deleteOne({userId}, {session});
    return result.deletedCount === 1;
  }

  async findOne(filter: FilterQuery<ITwoFactorAuth>, options?: {select?: string}) {
  ...
  async findOneAndUpdate(
    ...
    return await this.model.findOneAndUpdate(filter, update, {new: true, session}).lean();
  }

  async addAuditLog(userId: string, log: AuditLog) {
    return await this.model.updateOne({userId}, {$push: {auditLogs: log}});
  }

  async updateBackupCodes(userId: string, backupCodes: string[], session?: ClientSession) {
    return await this.model.updateOne({userId}, {$set: {backupCodes}}, {session});
  }
  ...
  async resetRecoveryAttempts(userId: string) {
    return await this.model.updateOne({userId}, {$set: {recoveryAttempts: 0, lastUsed: null}});
  }
