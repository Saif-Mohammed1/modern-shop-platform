...
export class TokensService {
  ...
  constructor() {
    this.RefreshExpiresAt = new Date(
      Date.now() +
        Number(process.env.JWT_REFRESH_TOKEN_COOKIE_EXPIRES_IN || 7) * 24 * 60 * 60 * 1000,
    );
  ...
  hashFingerprint = (fp: string) => crypto.createHash('sha256').update(fp).digest('hex');
  getResetPasswordTokenExpiry(): Date {
    return new Date(
      Date.now() + Number(process.env.HASHED_RESET_PASSWORD_SECRET_EXPIRES_IN || 10) * 60 * 1000,
    );
  }

  getVerificationTokenExpiry(): Date {
    return new Date(
      Date.now() + Number(process.env.HASHED_VERIFICATION_SECRET_EXPIRES_IN || 10) * 60 * 1000,
    );
  }
