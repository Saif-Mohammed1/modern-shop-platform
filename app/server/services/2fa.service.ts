...
import type {AuditLog, ITwoFactorAuth, SecurityMetadata} from '../models/2fa.model';
...
export class TwoFactorService {
  constructor(
    private readonly repository: TwoFactorRepository = new TwoFactorRepository(TwoFactorAuthModel),
  ...
  async initialize2FA(userId: string, metadata: SecurityMetadata) {
    ...
    if (existing2FA) throw new AppError(TwoFactorTranslate[lang].error.alreadyInitialized, 400);
  ...
  async verify2FA(userId: string, token: string, metadata: SecurityMetadata) {
    ...
    if (!verified) {
      ...
      throw new AppError(TwoFactorTranslate[lang].error.invalidVerificationCode, 401);
    }
  ...
  async verifyBackupCode(userId: string, code: string, metadata: SecurityMetadata) {
    const twoFA = await this.repository.findOne({userId});
    if (!twoFA) throw new AppError(TwoFactorTranslate[lang].error.notConfigured, 404);
    ...
    const updatedCodes = twoFA.backupCodes.filter((_, index) => index !== validCodeIndex);
    await this.repository.updateBackupCodes(userId, updatedCodes);
    await this.repository.addAuditLog(userId, this.createAuditLog('BACKUP_CODE_USED', metadata));
  ...
  async disable2FA(userId: string, metadata: SecurityMetadata) {
    ...
    if (!twoFA || !user) throw new AppError(TwoFactorTranslate[lang].error.notConfigured, 400);
    ...
    await this.repository.addAuditLog(userId, this.createAuditLog('2FA_DISABLED', metadata));
  ...
  async regenerateBackupCodes(userId: string, metadata: SecurityMetadata) {
    ...
    await this.repository.updateBackupCodes(userId, newCodes.map(this.cryptoService.hashCode));
  ...
  async getAuditLogs(userId: string) {
    const twoFA = await this.repository.findOne({userId}, {select: 'auditLogs'});
    return twoFA?.auditLogs.reverse().slice(0, 10) || [];
  }

  async validateBackupCodes(email: string, codes: string[], deviceInfo: DeviceInfo) {
    ...
    const twoFA = await this.repository.findOne({userId: user._id}, {select: '+backupCodes'});
    ...
    try {
      ...
      const updatedCodes = twoFA.backupCodes.filter((_, index) => !matchedIndices.includes(index));

      if (updatedCodes.length !== twoFA.backupCodes.length - 5) {
        await session.abortTransaction();
        throw new AppError(TwoFactorTranslate[lang].error.validationFailed, 400);
      }

      await this.repository.updateBackupCodes(user._id.toString(), updatedCodes, session);
  ...
  private async handleFailedAttempt(userId: string, metadata: SecurityMetadata) {
    await Promise.all([
      this.repository.incrementRecoveryAttempts(userId),
      this.repository.addAuditLog(userId, this.createAuditLog('2FA_FAILED_ATTEMPT', metadata)),
    ]);
  }
  async verifyLogin2FA(tempToken: string, code: string, metadata: SecurityMetadata) {
  ...
  private async handleSuccessfulVerification(userId: string, metadata: SecurityMetadata) {
    await Promise.all([
      this.userService.enable2FA(userId),
      this.repository.resetRecoveryAttempts(userId),
      this.repository.addAuditLog(userId, this.createAuditLog('2FA_SUCCESS', metadata)),
    ]);
  }
  ...
  private checkLockoutStatus(twoFA: ITwoFactorAuth) {
    if (
      ...
      twoFA.lastUsed &&
      Date.now() - twoFA.lastUsed.getTime() < SECURITY_CONFIG.LOCKOUT_MINUTES * 60 * 1000
    ) {
      throw new AppError(
        TwoFactorTranslate[lang].locked(String(SECURITY_CONFIG.LOCKOUT_MINUTES)),
        429,
      );
    }
  }
