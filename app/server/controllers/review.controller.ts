...
class ReviewController {
  ...
  async getRatingDistributionByProductId(req: NextRequest): Promise<NextResponse> {
    const distribution = await this.reviewService.getRatingDistributionByProductId(
      req.id as string,
    );
    return NextResponse.json(distribution, {status: 200});
  }
  ...
  async getMyReviews(req: NextRequest) {
    const reviews = await this.reviewService.getMyReviews((req.user as IUser)._id.toString(), {
      query: req.nextUrl.searchParams,
      populate: true,
    });
    return NextResponse.json(reviews, {status: 200});
