...
class AuthController {
  ...
  async requestEmailChange(req: NextRequest) {
    ...
    const deviceInfo = await getDeviceFingerprint(req);
    await this.userService.requestEmailChange(req.user?._id.toString(), email, deviceInfo);
    return NextResponse.json(
      {
  ...
  async verifyEmail(req: NextRequest) {
    ...
    const deviceInfo = await getDeviceFingerprint(req);
    await this.userService.verifyEmail(req.user?._id.toString(), code, deviceInfo);
    return NextResponse.json(
      {
  ...
  async sendNewVerificationCode(req: NextRequest) {
    ...
    const deviceInfo = await getDeviceFingerprint(req);
    await this.userService.sendVerificationCode(req.user?._id.toString(), deviceInfo);
    return NextResponse.json(
      {
  ...
  async updateLoginNotificationSent(req: NextRequest) {
    ...
    const body = await req.json();
    const result = UserValidation.validateLoginNotificationSent(body.loginNotificationSent);
    await this.userService.updateLoginNotificationSent(req.user?._id.toString(), result);
    return NextResponse.json(
      {
  ...
  async getActiveSessions(req: NextRequest) {
    ...
    const sessions = await this.userService.getActiveSessions(req.user?._id.toString());
