...
const DeviceSessionCard = ({
  ...
  const handleRevoke = async () => {
    ...
    try {
      await api_client.delete(`/customers/device/${session._id}`);
  ...
  const StatusBadge = () => {
    ...
    return (
      ...
      >
        <span
          className={`w-2 h-2 rounded-full animate-pulse ${session.is_active ? "bg-green-500" : "bg-gray-300"}`}
        />
          ...
          <AnimatePresence>
            {isHovered || session.is_active ? (
              ...
              >
                ...
                >
                  <MdSecurity
                    className={`text-lg ${session.device_info.is_bot ? "text-red-500" : "text-green-500"}`}
                  />
                </Tooltip>
              </motion.div>
            ) : null}
