...
const UserOrderTracking = ({ order }: { order: OrdersType }) => {
  if (!order) {
    return (
  ...
  return (
    ...
    <div className="border p-6 rounded shadow-lg bg-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <h3 className="text-xl font-semibold mb-2">
            {accountOrdersTranslate[lang].orderTracking.shippingInfo.title}
          </h3>
          ...
          <p>
            {accountOrdersTranslate[lang].orderTracking.shippingInfo.phone}:{" "}
              ...
              >
                <p>
                  <span className="font-bold">
                    {accountOrdersTranslate[lang].orderTracking.items.name}:
                  </span>{" "}
                  {item.name}
                </p>
                <p>
                  <span className="font-bold">
                    {accountOrdersTranslate[lang].orderTracking.items.quantity}:
                  </span>{" "}
                  {item.quantity}
                </p>
                <p>
                  <span className="font-bold">
                    {accountOrdersTranslate[lang].orderTracking.items.price}:
                  </span>{" "}
                  ${item.price}
                </p>
                <p>
                  <span className="font-bold">
                    {accountOrdersTranslate[lang].orderTracking.items.discount}:
                  </span>{" "}
                  {item.discount > 0
                    ? `$${item.discount}`
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mt-6">
        ...
        <p className="w-full sm:w-1/2">
          <h3 className="text-xl font-semibold mb-2">
            {accountOrdersTranslate[lang].orderTracking.orderStatus.totalPrice}
          </h3>
